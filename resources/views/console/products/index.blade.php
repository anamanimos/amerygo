...
<div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
    <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
        <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
            <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">Manajemen Produk</h1>
        ...
        <div class="d-flex align-items-center gap-2 gap-lg-3">
            ...
            @if($currentCount >= $limit)
                <button class="btn btn-sm fw-bold btn-secondary" disabled data-bs-toggle="tooltip" title="Batas maksimal produk tercapai">
                    <i class="ki-duotone ki-plus fs-2"></i> Tambah Produk
                </button>
            @else
                <a href="{{ route('console.products.create') }}" class="btn btn-sm fw-bold btn-primary">
                    <i class="ki-duotone ki-plus fs-2"></i> Tambah Produk
                </a>
            @endif
        </div>
            ...
            <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input type="text" data-kt-product-table-filter="search" class="form-control form-control-solid w-250px ps-12" placeholder="Cari Produk..." />
                    </div>
                </div>
            </div>
                        ...
                        <tr>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="symbol symbol-50px me-5">
                                        <span class="symbol-label bg-light">
                                            @if($product->thumbnail)
                                                <img src="{{ asset($product->thumbnail) }}" class="h-100 w-100 align-self-center rounded" style="object-fit:cover;" alt="" />
                                            @else
                                    ...
                                    <div class="d-flex flex-column">
                                        <a href="{{ route('console.products.edit', $product->id) }}" class="text-gray-800 text-hover-primary mb-1 fw-bold fs-5">{{ $product->name }}</a>
                                        <span class="text-muted fs-7">/{{ $product->slug }}</span>
                                    </div>
                                </div>
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                @if($product->discount_price > 0)
                                    ...
                                    <div class="text-muted text-decoration-line-through fs-7">Rp {{ number_format($product->price, 0, ',', '.') }}</div>
                                @else
                                    <div class="text-gray-900 fw-bold">Rp {{ number_format($product->price, 0, ',', '.') }}</div>
                                @endif
                            </td>
                            ...
                            <td class="text-end">
                                ...
                                <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                                    <div class="menu-item px-3">
                                        <a href="{{ route('console.products.edit', $product->id) }}" class="menu-link px-3">Edit</a>
                                    </div>
                                    <div class="menu-item px-3">
                                        <form action="{{ route('console.products.destroy', $product->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini?');">
                                            @csrf
                                            @method('DELETE')
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const table = document.getElementById('kt_products_table');
        if (table) {
